...
# Clear cache every time we check (for real-time updates)
_cache_valid = False

def get_steward_proxy_enabled():
    """Get STEWARD_PROXY_ENABLED flag - always fetches current value."""
    global _cache_valid
    _cache_valid = False  # Force fresh check
    return get_flag('STEWARD_PROXY_ENABLED', 'STEWARD_PROXY_ENABLED', default=False)

def get_profile_picture_enabled():
    """Get PROFILE_PICTURE_ENABLED flag - always fetches current value."""
    global _cache_valid
    _cache_valid = False  # Force fresh check
