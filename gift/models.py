...
class WikiUser(AbstractUser):
    ...
    profile_picture_web = models.ImageField(
        upload_to='profile_pictures/web/',
        blank=True,
        null=True,
        help_text='Web version (400x400)'
    )
    scraped_page_selected = models.BooleanField(
        default=False,
        help_text='Whether user has selected a scraped wiki page to import'
    )
            ...
            if self.profile_picture:
                ...
                if hasattr(self.profile_picture, 'read'):
                    ...
                    if not os.path.exists(file_path):
                        ...
                        import logging
                        logger = logging.getLogger(__name__)
                        logger.warning(f"Profile picture file not found: {file_path}, clearing reference")
...
class WishList(models.Model):
    ...
    managers = models.ManyToManyField(WikiUser, related_name='managed_wishlists', blank=True,
...
class FeatureFlag(models.Model):
    ...
    """
    name = models.CharField(max_length=100, unique=True, help_text="Flag name (e.g., STEWARD_PROXY_ENABLED)")
