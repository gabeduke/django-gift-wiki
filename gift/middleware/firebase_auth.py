...
def _get_firebase_auth():
    ...
    try:
        ...
        if not firebase_admin._apps:
            ...
            try:
                ...
                if cred_path and os.path.exists(cred_path):
                    ...
                    _firebase_app = firebase_admin.initialize_app(cred)
                    logger.info(f"Firebase Admin SDK initialized with credentials file: {cred_path}")
                else:
                    logger.warning("Firebase Admin SDK not configured - Firebase auth will not work. "
        ...
        logger.error("Firebase Admin SDK not installed - Firebase auth will not work. "
...
class FirebaseAuthBackend(ModelBackend):
    ...
    def authenticate(self, request, username=None, password=None, **kwargs):
        ...
        try:
            ...
            try:
                decoded_token = firebase_auth.verify_session_cookie(session_cookie, check_revoked=False)
                ...
                logger.warning(f"Session cookie verification failed ({e}), attempting ID token verification fallback")
                try:
                    decoded_token = firebase_auth.verify_id_token(session_cookie, check_revoked=False)
...
class FirebaseAuthMiddleware:
    ...
    def __init__(self, get_response):
        ...
        if allowed_users_env:
            ...
            self.allowed_users = {email.strip().lower() for email in allowed_users_env.split(',') if email.strip()}
            logger.info(f"Allowlist enabled with {len(self.allowed_users)} allowed users (from environment)")
        else:
    ...
    def __call__(self, request):
        ...
        if request.path in ['/health/', '/metrics', '/metrics/', '/auth.html'] or request.path.startswith('/auth.html'):
        ...
        logger.info(f"Request to {request.path} - Cookies present: {all_cookies}, Has __session: {bool(session_cookie)}, Cookie header length: {len(cookie_header)}")
        ...
        if not session_cookie:
            ...
            if hasattr(request, 'user') and request.user.is_authenticated:
                logger.info(f"User {request.user.email} already authenticated in Django session for {request.path}")
                return self.get_response(request)
            logger.info(f"No Firebase session cookie found for {request.path} - all cookies: {all_cookies}")
            return self.get_response(request)
        ...
        if not self._is_user_allowed(email):
            logger.warning(
                f"Access denied for user {email} - not in allowlist. "
        ...
        try:
            ...
            logger.info(f"Authenticated user via Firebase (stateless): {authenticated_user.username} ({email})")
        except Exception as e:
