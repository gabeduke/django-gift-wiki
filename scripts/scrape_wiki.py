...
def is_navigation_text(text):
    ...
    nav_keywords = [
        ...
        'lets say you want to get a gift',
        'oh, no! you forgot',
        'to edit this page',
        'wiki',
        'gift wiki$',  # $ means end of string
    ]
...
def extract_list_items(soup):
    ...
    for tag in soup.find_all(['s', 'strike', 'del']):
        ...
        if text and len(text) > 2 and not is_navigation_text(text):
            items.append({'name': text, 'purchased': True, 'raw_html': str(tag)})
    ...
    for ul in soup.find_all(['ul', 'ol']):
        ...
        for li in ul.find_all('li', recursive=False):
            ...
            if text and len(text) > 2 and not is_navigation_text(text):
                ...
                items.append({'name': text, 'purchased': is_purchased, 'raw_html': str(li)})
    ...
    for p in soup.find_all('p'):
        ...
        if (':' in text and len(text) > 10) or (len(text) > 5 and len(text) < 200):
            ...
            items.append({'name': text, 'purchased': is_purchased, 'raw_html': str(p)})
    ...
    if main_content:
        ...
        for nav_elem in main_content.find_all(
            class_=re.compile(r'nav|sidebar|toolbar|footer', re.I)
        ):
            nav_elem.decompose()
        ...
        for div in main_content.find_all('div', recursive=True):
            ...
            if text and len(text) > 3 and len(text) < 300 and not is_navigation_text(text):
                ...
                items.append({'name': text, 'purchased': is_purchased, 'raw_html': str(div)})
...
def scrape_page(url):
    ...
    try:
        ...
        for nav in main_content.find_all(['nav', 'header', 'footer']):
            nav.decompose()
        for nav_elem in main_content.find_all(
            class_=re.compile(r'nav|sidebar|toolbar|footer|header', re.I)
        ):
            nav_elem.decompose()
        ...
        for ul in main_content.find_all(['ul', 'ol']):
            ...
            for li in ul.find_all('li', recursive=False):
                ...
                if text and len(text) > 2 and not is_navigation_text(text):
                    is_purchased = bool(li.find(['s', 'strike', 'del']))
                    raw_items.append({'name': text, 'purchased': is_purchased, 'raw_html': str(li)})

        # Strategy 2: Look for strikethrough items (purchased items)
        for tag in main_content.find_all(['s', 'strike', 'del']):
            text = clean_text(tag.get_text())
            if text and len(text) > 2 and not is_navigation_text(text):
                raw_items.append({'name': text, 'purchased': True, 'raw_html': str(tag)})
        ...
        for p in main_content.find_all('p'):
            ...
            if text and len(text) > 3 and len(text) < 500 and not is_navigation_text(text):
                ...
                if 'at ' in text and ('am on' in text or 'pm on' in text):
                    continue
                is_purchased = bool(p.find(['s', 'strike', 'del']))
                raw_items.append({'name': text, 'purchased': is_purchased, 'raw_html': str(p)})
            ...
            normalized_items.append(
                {
                    ...
                    or item['name'],  # Use full text as description if no separate description
        ...
        return {'url': url, 'title': None, 'items': [], 'item_count': 0, 'error': str(e)}
            ...
            is_wiki_page = (
                (
                    ...
                    or (
                        parsed.path
                        and parsed.path != '/'
                        and not parsed.path.startswith('/w/')
                        and parsed.path not in ['/browse', '/FrontPage', '/request_access.php']
                        and '?' not in parsed.path
                    )
                )  # Old-style PBworks URLs
                and '/FrontPage' not in full_url
                and '/browse' not in full_url
            )
...
def scrape_frontpage():
    ...
    try:
        ...
        known_pages = [
            {
                'url': 'http://wikichristmas.pbworks.com/w/page/14269060/Gabe',
                'text': 'Gabe',
                'name': 'Gabe',
            },
            {
                'url': 'http://wikichristmas.pbworks.com/w/page/137156280/Arlo',
                'text': 'Arlo',
                'name': 'Arlo',
            },
            {
                'url': "http://wikichristmas.pbworks.com/w/page/123317856/Luna%20Mae's%20Lavish%20List",
                'text': "Luna Mae's Lavish List",
                'name': 'Luna Mae',
            },
            {
                'url': 'http://wikichristmas.pbworks.com/w/page/14269093/John',
                'text': 'John',
                'name': 'John',
            },
            # Old-style PBworks URLs (without /w/page/)
            {
                'url': 'http://wikichristmas.pbworks.com/Goodies+for+Gladys',
                'text': 'Goodies for Glady',
                'name': 'Glady',
            },
            {
                'url': 'http://wikichristmas.pbworks.com/Jack%27s-List',
                'text': "Jack's List",
                'name': 'Jack',
            },
            {'url': 'http://wikichristmas.pbworks.com/Jim', 'text': 'Jim', 'name': 'Jim'},
            {
                'url': 'http://wikichristmas.pbworks.com/Joseph',
                'text': "Joseph's Just-So Gifts",
                'name': 'Joseph',
            },
            {
                'url': 'http://wikichristmas.pbworks.com/Skippy%27s',
                'text': "Skippy's",
                'name': 'Skippy',
            },
            {
                'url': 'http://wikichristmas.pbworks.com/Alternative-gifts',
                'text': 'Alternative gifts',
                'name': 'Alternative gifts',
            },
            {
                'url': 'http://wikichristmas.pbworks.com/Rachel-Ellison',
                'text': "Rachel's Request Box",
                'name': 'Rachel',
            },
        ]
...
def main():
    ...
    for page in sorted_pages[:10]:
        if page['item_count'] > 0:
            print(
                f'  {page.get("title", page.get("link_text", "Unknown"))}: {page["item_count"]} items'
            )
